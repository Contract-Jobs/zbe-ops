...
export interface BulkEquipmentMovementFormProps {
...
export function BulkEquipmentMovementForm({
  equipmentId,
  defaultSource,
  defaultDestination,
  fixedSource,
  ...
  noBg,
  onCancel,
}: BulkEquipmentMovementFormProps) {
  ...
  const { data: itemsData } = useInventoryItems({ category: "equipment", limit: 50, tracking: 'quantity' });
  ...
  const [selectedMaterialId, setSelectedMaterialId] = useState<string>(equipmentId ?? "");
  const mId = equipmentId || selectedMaterialId;
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (type === "purchase") {
        ...
        // The generic /inventory-movements purchase's autoCreateItem is
        // documented as category:"material" only (equipment's own
        // auto-create lives under the individual-equipment purchase
        // payload instead) — so a brand-new bulk-equipment catalog item
        // has to be created explicitly first, then purchased by id.
        let purchaseItemId = mId === "new" ? undefined : mId;
        if (mId === "new") {
          if (!matName.trim()) throw new Error("Equipment name is required");
          const { data: newItem } = await createItemMutation.mutateAsync({
            name: matName,
            unit: matUnit || "pcs",
            category: "equipment",
            tracking: "quantity",
